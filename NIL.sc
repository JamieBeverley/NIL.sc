...
Server.default.waitForBoot({
	...
	SynthDef(\grain,{
		|amp,out=0,spectralCentroid=0,rms=0,pitch=0,clarity=0,strength=0,turbidity=0,corpus=0,attack=0.01,release=0.01|

		SendReply.kr(Line.kr(-1,1,dur:0.01),"/grain",[out,amp,corpus,attack,release,rms,spectralCentroid,pitch,clarity,strength,turbidity]);
	...
	cmdPFunc = {
		Tdef(\ugh,{
			...
			OSCdef(\playGrain,{
				...
				features["rms"] = msg[8].asFloat;
				features["spectralCentroid"] = msg[9].asFloat;
				features["pitch"] = msg[10].asFloat;
				features["clarity"] = msg[11].asFloat;
				features["strength"] = msg[12].asFloat;
				features["turbidity"] = msg[13].asFloat;
